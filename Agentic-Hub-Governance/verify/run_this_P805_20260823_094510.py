"""PEH verification: WO-P805-E2.001 Outlook OAuth2 support.

Validates the new/changed files compile under p140 with warnings-as-errors,
that msal is importable, and runs the project's own unittest suite
(tests/test_imap_mover.py + tests/test_oauth2_outlook.py) without touching
any live keyring entry, network call, or browser.
...
def import_check() -> list[str]:
    """Confirm msal is installed and importable in p140."""
    result = subprocess.run(
        [PROJECT_PYTHON, "-c", "import msal; print(msal.__version__)"],
        capture_output=True,
        text=True,
    )
    if result.returncode != 0:
        return [f"msal import failed: {result.stderr.strip()}"]
    return []
