# WO-P000-E20.001 verify (v1.1): compile check + regression tests +
# real re-run against P_000's actual lessons.md after the chunking fix.
...
def run_regression_tests() -> str | None:
    """Directly calls the two test_domain.py assertions -- no pytest
    dependency required for this self-contained verify script. Returns
...
def main() -> int:
    ...
    if compile_error:
        ...
        write_done("FAIL", 1)
        return 1
    print("Compile check: PASS (7 files, -W error::SyntaxWarning)")
    ...
    if test_error:
        ...
        write_done("FAIL", 1)
        return 1
    print("Regression tests: PASS (2/2 -- scattered-no-flag, concentrated-flags)")
    ...
    scratch_output = VERIFY_DIR / "lessons_audit_status_SCRATCH_verify_v2.json"
    ...
    print(f"Classification counts (post-chunking-fix): {counts}")
    for flag in status.flags:
        ...
        print(f"  {flag.lesson_id} [{flag.lesson_title[:40]}]: {flag.classification} "
              f"(max_shared={flag.shared_term_count}) top_match={excerpt!r}")
