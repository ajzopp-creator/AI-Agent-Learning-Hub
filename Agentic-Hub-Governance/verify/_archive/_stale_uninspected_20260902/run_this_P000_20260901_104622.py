# WO-P000-E20.001 verify (v1.3): compile check + 6 regression tests +
# real re-run against P_000's actual lessons.md after corpus-document-
# frequency filtering (drops words pervasive across the reference
# corpus, not just fixed-list English stopwords).
...
def run_regression_tests() -> str | None:
    ...
    test_names = (
        ...
        "test_pervasive_corpus_words_do_not_count_toward_a_flag",
        "test_rare_corpus_word_is_not_filtered",
    )
...
def main() -> int:
    ...
    if test_error:
        ...
        write_done("FAIL", 1)
        return 1
    print("Regression tests: PASS (6/6)")
    ...
    scratch_output = VERIFY_DIR / "lessons_audit_status_SCRATCH_verify_v4.json"
    ...
    print(f"Classification counts (post-doc-frequency-filter): {counts}")
    for flag in status.flags:
