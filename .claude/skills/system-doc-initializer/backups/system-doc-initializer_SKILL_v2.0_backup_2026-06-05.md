...
`📌 To add to Claude Desktop Project: Project Settings → Add Content → Upload this file`
...
**Trigger:** any session where file output is needed but runtime environment may be Linux sandbox (claude.ai web) while destination is Windows (local filesystem).

**Rule 1 — Detect environment automatically**

- Claude Desktop (Windows-MCP present) → direct file write to Windows paths via PowerShell
- claude.ai web (no Windows-MCP) → Linux sandbox, no local filesystem access

**Rule 2 — Linux sandbox file creation workflow**

When creating files in Linux sandbox with Windows destinations:
1. Create file in `/mnt/user-data/outputs/` (always accessible)
2. Use `present_files` to show all created files
3. Provide a manifest table with Windows destination paths
4. User downloads and manually places files (copy-paste or file explorer)

**Rule 3 — File delivery format for Linux→Windows**

```
CREATED IN: /mnt/user-data/outputs/
TARGET LOCATIONS (copy files to these paths):

| File | Windows Destination |
|------|-------------------|
| P_000_INIT_v1.0.md | C:\Users\Trader\...\P_000\docs\ |
| P_300_INIT_v3.1.md | C:\Users\Trader\...\P_300\docs\prompts\ |
```

**Rule 4 — Automate the file routing (optional)**

Create a PowerShell helper script that copies all files from outputs to their Windows destinations. User can run manually after downloading.

**Rule 5 — Skill files to .claude\skills\ ONLY**

When creating or updating SKILL.md files:
- Target: `C:\Users\Trader\AI-Agent-Learning-Hub\.claude\skills\[skill-name]\SKILL.md`
- Create in `/mnt/user-data/outputs/` during claude.ai web session
- User downloads and manually places in `.claude\skills\` folder
- User uploads to Desktop Claude Project via Project Settings → Add Content

Never write skill files to `/mnt/skills/` (read-only system directory).
...
*Last Updated: June 4, 2026 — Added Protocol E (Mixed-Environment File Handling). 
Solves recurring issue where Linux sandbox files don't transfer to Windows.
Provides manual placement workflow + corrected Protocol B Rule 3 for skill file location.
CRITICAL FIX: Skill files go to .claude\skills\ ONLY, never shared_resources.
Applies to any claude.ai web session creating files for Windows destinations.*

---

## Error Corrections Log

### EC-002 — Skill File Placed in Wrong Location (2026-06-04)

**Severity:** High  
**Category:** Protocol B Rule 3 violation + Protocol E Rule 5 missing

**What happened:**
Claude updated skill in `/mnt/skills/user/system-doc-initializer/SKILL.md` (read-only system directory)
instead of creating a downloadable version in `/mnt/user-data/outputs/` for manual placement.

**Root cause:**
Rule 3 had placeholder "shared_resources\skills\" as wrong example. Protocol E (mixed environments)
did not explicitly state skill file destination. Claude mistakenly assumed editing system skill
would auto-sync to user's Desktop Claude Project.

**Correct rule:**
All skill files MUST be placed at (and ONLY at):
```
C:\Users\Trader\AI-Agent-Learning-Hub\.claude\skills\[skill-name]\SKILL.md
```

When updating skills in claude.ai web session:
1. Create file in `/mnt/user-data/outputs/skill-name_SKILL.md`
2. User downloads to local machine
3. User navigates to `.claude\skills\[skill-name]\` folder
4. User manually places SKILL.md (replacing old version)
5. User uploads to Desktop Claude Project via Project Settings → Add Content

Never write to `/mnt/skills/` (system read-only), `shared_resources\skills\`, or assume
auto-sync behavior.

**Fix applied:**
Protocol B Rule 3 corrected to `.claude\skills\` only.
Protocol E Rule 5 added (skill file explicit handling).
EC-002 added to log.
