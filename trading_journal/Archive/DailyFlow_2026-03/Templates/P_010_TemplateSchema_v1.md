...
# 📅 Daily Flow — <% tp.date.now("MMMM,dddd, MMMM D, YYYY") %>
...
try {
  ...
  tR += `> *${data[0].bookname} ${data[0].chapter}:${data[0].verse}*\n> "${data[0].text}"`;
} catch(e) {
  tR += '> _Scripture could not be loaded — check your internet connection._';
}
%>
...
<%*
try {
  const res = await fetch('https://v2.jokeapi.dev/joke/Any?safe-mode&type=single&blacklistFlags=nsfw,racist,sexist,explicit');
  const data = await res.json();
  tR += `> 😂 ${data.joke}`;
} catch(e) {
  tR += '> _Humor could not be loaded — check your internet connection._';
}
%>
