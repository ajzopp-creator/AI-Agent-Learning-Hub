...
**Project ID:** P_400
**Version:** 1.0 (Finalized Architecture — Enhancement 1: Signal File Handoff, Phased Implementation Plan)
**Last Updated:** 2026-06-03 (v1.0 — Finalized)
**Maintained By:** Anthony Zoppi
**Status:** Phase 1 Finalized — Manual Execution Prototype (Enhancement 1 Ready for P_115 Implementation)
...
**Enhancement 1 (v0.4):** P_115 now outputs a structured JSON signal-packet file. P_400 reads the file (instead of inline chat) in STEP 1, establishing the machine-readable interface for the eventual full P_115↔P_400 split (Enhancement 2, future).
...
#### Component 1: Signal Ingestion (Enhancement 1)
- **Responsibility (v0.4 Enhancement 1):** Read a JSON signal-packet file from `TradeOrderManagement/signals/` (written by P_115 STEP 1). Parse the structured fields: symbol, guideline entry, guideline stop, guideline target, signal source, strategy, horizon, confidence. **Then check the open-position book for any non-CLOSED P_400 record in the same symbol** — if found, do not silently build a second order; route to the scale-in / replace / skip sub-flow (Section 8.4).
- **Inputs:** P_115 signal-packet JSON file from Obsidian vault (`TradeOrderManagement/signals/YYYY-MM-DD_SYMBOL_signal.json`). See §2.5 for the signal-packet schema.
- **Outputs:** Structured signal payload (symbol, source, signal_date, guideline_entry, guideline_stop, guideline_target, horizon, confidence, signal_source_link for audit trail)
- **Tools Used:** File read, JSON parse
- **Dependencies:** P_115 Enhancement 1 (file-based signal output) must be in place; signal file must conform to P_115_P400_SIGNAL_PACKET_SCHEMA_v1_0
...
### Version History
#### v0.4 — 2026-06-02
**Release Type:** Enhancement 1 integration (signal-file handoff interface)

**Added:**
- §2.5 P_115→P_400 Interface specification with JSON signal-packet schema reference
- Component 1 updated to read JSON signal-packet file instead of inline chat
- Config: signal_file_dir parameter for vault location
- Roadmap: Enhancement 2 (future full P_115↔P_400 split) documented

**Summary:** Enhancement 1 establishes the machine-readable interface between P_115 and P_400. P_115 writes structured JSON to `TradeOrderManagement/signals/`; P_400 STEP 1 reads it. This is the bridge to Enhancement 2 (full architectural split) and implements recommendation #1 from the Process Evaluation doc.
...
## 8.5 Work Order Standard & Governance

### 8.5.1 Purpose

Claude owns all work orders across P_115, P_300, P_400, and P_800. Work orders track cross-project dependencies and are tagged to enhancement phases so progress is transparent and blockers are identified early.

### 8.5.2 Work Order Schema

**Filename:** WO-[SOURCE]-[PHASE].[SEQ].md  
**Location:** C:\Users\Trader\AI-Agent-Learning-Hub\projects\P_400_TradeOrderManagement\work_orders\PHASE_[PHASE]\

**Fields:**

| Field | Type | Required | Notes |
|---|---|---|---|
| ID | string | Yes | Format: WO-[PROJECT]-[PHASE].[SEQ] |
| Source Project | enum | Yes | P_115, P_300, P_400, P_800 |
| Phase/Enhancement | enum | Yes | E0 (design), E1 (signal emit), E2 (full split), E3+ (future) |
| Task | string | Yes | One-line description of the work |
| Input | string | Yes | What data/schema is required |
| Output | string | Yes | Deliverable, format, path |
| Status | enum | Yes | PENDING, IN_PROGRESS, BLOCKED, COMPLETE |
| Owner | enum | Yes | Anthony or Claude |
| Depends On | list | No | Other WO IDs |
| Due | date | No | ISO format (YYYY-MM-DD) |
| Notes | string | No | Blockers, decisions, context |

### 8.5.3 Current Work Order Sequence — Phase E1 IN_PROGRESS

**Phase E0 (Design) — COMPLETE:**
- WO-P400-E0.001: Architecture v0.4 design (COMPLETE, Claude)
- WO-P800-E0.001: Signal-packet schema v1.0 (COMPLETE, Claude)

**Phase E1 (Signal Emission) — IN_PROGRESS:**
- WO-P800-E1.001: P400SignalRecord + write_handler fork (COMPLETE, Claude)
- WO-P115-E1.001: P_115 signal_emitter code (PENDING, Anthony) — Depends: WO-P800-E1.001
- WO-P300-E1.001: P_300 signal_emitter code (PENDING, Anthony) — Depends: WO-P800-E1.001
- WO-P400-E1.001: P_400 STEP 1 JSON reader (PENDING, Claude) — Depends: WO-P115-E1.001, WO-P300-E1.001

**Phase E2 (Architectural Split) — DESIGN:**
- WO-P115-E2.001: Remove P_115 STEP 2 (PENDING, Anthony) — Depends: WO-P115-E1.001
- WO-P300-E2.001: Remove P_300 STEP 2 (PENDING, Anthony) — Depends: WO-P300-E1.001
- WO-P400-E2.001: JSON as sole upstream source (PENDING, Claude) — Depends: WO-P400-E1.001

### 8.5.4 Management Rules

1. **Ownership:** Claude creates, tracks, and updates all work orders across all projects.
2. **Granularity:** One WO per deliverable. Split if task spans multiple files or milestones.
3. **Dependency Tracking:** Always declare Depends On to expose critical paths.
4. **Status Updates:** Update every session. If a WO blocks downstream work, escalate immediately.
5. **Phase Advancement:** A phase cannot advance until all WOs in current phase are COMPLETE.
6. **Storage:** work_orders/PHASE_[PHASE]/ folders. Index updated weekly.
7. **Cross-Project Calls:** Requests from other projects become new WOs with explicit dependencies.
...
| ID-006 | Signal file validation must handle malformed or incomplete JSON | Medium | STEP 1 validates file structure and required fields; reports upstream defects; never tries to patch incomplete signals | Open (Enhancement 1 implementation) |
...
# Last Updated: 2026-06-02 (v0.4 — Enhancement 1: Signal File Handoff)
...
signal_file_dir               = "<vault-root>/TradeOrderManagement/signals/"
...
| 1.0 | 2026-06-03 | Anthony Zoppi | FINALIZED — Enhancement 1 ready for P_115 implementation. Phase 1 manual-execution prototype locked. E2/E3 roadmap documented. Work order governance in place.. P_115 writes JSON signal-packet to TradeOrderManagement/signals/; P_400 STEP 1 reads it. Establishes machine contract for future Enhancement 2 (full P_115↔P_400 split). Ref: P_115_P400_SIGNAL_PACKET_SCHEMA_v1_0 |
