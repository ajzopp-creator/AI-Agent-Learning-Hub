...
def test_quant_blocks_rr_below_min():
    ...
    # WO-P400-E2.016: message must disclose the two different bases
    assert "spread-adjusted" in v.reason_detail
    assert "clean/guideline basis" in v.reason_detail
...
def test_tape_pass():
    v = tape_vote(price_delay_seconds=30, market_open=True, price_basis="live",
                  adverse_drift_pct=0.0, rr_after_drift=2.5)
    assert v.decision == Decision.PASS

def test_tape_blocks_stale_price():
    v = tape_vote(price_delay_seconds=200, market_open=True, price_basis="live",
                  adverse_drift_pct=0.0, rr_after_drift=2.5)
    assert v.decision == Decision.BLOCK
...
def test_verdict_approved_all_pass():
    votes = [
        ...
        macro_vote(False),
        tape_vote(30, True, "live", 0.0, 2.5),
