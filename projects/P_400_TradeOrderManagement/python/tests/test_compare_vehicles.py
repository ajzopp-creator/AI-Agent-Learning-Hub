...
WO-P400-E5.006 (2026-08-11) adds NEITHER-disposal coverage: run_comparison()
now archives the packet and writes REVIEWED_NO_TRADE when neither vehicle
is viable, mirroring the existing ENTRY_MISSED disposal path.
"""
import json
import pytest
...
def test_run_comparison_neither_disposes_packet(tmp_path):
    """NEITHER (both vehicles fail R:R) must archive the packet and write a
    REVIEWED_NO_TRADE record -- Tony's call, 2026-08-11: once a signal
    reaches P_400 it is either a trade or it is not; P_400 does not hold a
    packet waiting to re-qualify later. Regression for the gap found live
    on HAL/VKTX (both NEITHER, both left sitting in the inbox undisposed
    before this fix)."""
    packet = _make_packet(entry=100.0, stop=99.0, target=100.5)  # R:R far below 2.0
    snap_path = _write_snapshot(tmp_path)
    chain_path = _write_chain(tmp_path, open_interest=86)  # also fails options council

    posture_mock, params_mock = _mock_infra()

    with patch("application.compare_vehicles.read_posture", return_value=posture_mock), \
         patch("application.compare_vehicles.read_params", return_value=params_mock), \
         patch("application.drop_signal.drop_signal", return_value=True) as spy:

        from application.compare_vehicles import run_comparison
        output = run_comparison(packet, snap_path, chain_path, cash_available=10000.0,
                                 trade_mode=TradeMode.REAL)

    assert "RECOMMENDATION: NEITHER" in output
    spy.assert_called_once_with(packet, "RR_INVALID", TradeMode.REAL)
    assert "REVIEWED_NO_TRADE" in output
    assert "drop_reason=RR_INVALID" in output
    assert "archived=OK" in output
