...
def make_snapshot(**kwargs) -> dict:
    """Minimal valid snapshot dict for SnapshotDict(**snapshot_raw).

    Mirrors make_snapshot() in test_evaluate_options.py verbatim -- same
    subsystem, same required-field set. next_earnings_date is left unset
    (None) so the MACRO earnings/binary-event check is a no-op and these
    tests exercise what they were written for: spread liquidity gates,
    sizing, and Pattern C rendering.

    Added 2026-08-05 (WO-P000-E13.001 Phase 2): evaluate_spread() gained a
    required snapshot_raw parameter and this file's three call sites were
    never updated, failing at run time. No assertion changed.
    """
    defaults = dict(
