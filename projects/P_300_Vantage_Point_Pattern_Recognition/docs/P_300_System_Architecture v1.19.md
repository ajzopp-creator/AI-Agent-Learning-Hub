...
+-----------------------+      +-----------------------+      +-----------------------+
|                       |      |                       |      |                       |
|  [Source Master Data] |----->|  [Handshake Protocol] |----->|   [models/mmddyygeminicatalog.db] |
...
* Pillar Isolation (v1.16): Strict separation between the Pattern Vault (The Brains) and the Trade Evaluation Pipeline.
* One-Way Data Valve: Historical patterns are vaulted permanently; Trade Evaluation data is transient and never pollutes the historical catalog.
* Filename Enforcement: The Pattern_ prefix serves as a physical gatekeeper for the historical vault.
...
P_300 Operational Pipeline: VantagePoint Pattern Recognition
...
This is the official operational protocol for processing VantagePoint CSV exports. It utilizes a four-step pipeline anchored to PowerShell wrappers to support both single-file and high-volume batch processing.

1. Pipeline A: The Brain Vault (Historical Ingest)
Source Folder: data\historical_patterns\
Filename Rule: Must start with Pattern_.
Processor: batch_ingest_orchestrator.py
Workflow: Read → Feature Engineering → Integrity Check → Archive to data\archive\.
Purpose: Building the long-term mathematical "DNA" library used for similarity matching.

2. Pipeline B: Daily Pattern Analysis (Operational Standard)
This workflow executes the daily trade decision engine. It is managed by a sequence of specialized PowerShell wrappers.
Step 1 — Ingest: Read incoming CSVs from data/live/ and write pattern instances into the catalog.
PowerShell
python python\ingest\ingest_vp_catalog.py --type EVAL_SET --db-path models\051026geminicatalog.db

Step 2 — Archive: Move processed CSVs to data\processed\ to maintain a clean workspace.
PowerShell
Move-Item -Path "data\live\*.csv" -Destination "data\processed\"

Step 3 — Pattern Match: Execute the IntelliScan engine to find historical analogs.
PowerShell
python python\matching\intelliscan.py

Step 4 — Confidence Report: The Smart Aggregator auto-detects the most recent IDs and generates the Z-Score Confidence Report.
PowerShell
python python\reporting\aggregator.py


3. PowerShell Wrappers (Utilities)
Wrappers are stored in python/utilities/ to simplify daily execution and eliminate path errors.
Wrapper
Stage
Purpose
P_300_00_WorkflowLauncher.ps1
Orchestrator
Master launcher for the 4-step daily pipeline.
P_300_05_ingest.ps1
Step 1
Runs ingest with EVAL_SET type against the latest DB.
P_300_10_ArchiveCSV.ps1
Step 2
Moves CSVs from data\live to data\processed.
P_300_20_PatternMatch.ps1
Step 3
Executes intelliscan.py similarity engine.
P_300_50_ConReport.ps1
Step 4
Executes aggregator.py (Smart Auto-ID Detection).

Sequential Execution Pattern:
PowerShell
& ".\python\utilities\P_300_05_ingest.ps1"
& ".\python\utilities\P_300_10_ArchiveCSV.ps1"
& ".\python\utilities\P_300_20_PatternMatch.ps1"
& ".\python\utilities\P_300_50_ConReport.ps1"


4. Batch Mode & Folder Roles
Batch Mode Logic: The system is designed for "One-to-Many" processing. When multiple CSVs (e.g., 10 symbols) are dropped into data/live/, the ingest script iterates through every file. The Aggregator then queries the database for all pattern IDs created in that specific session, producing a unified report for the entire batch.
Data vs. Code Separation:
To prevent technical debt, code remains under python/ while all artifacts and logs stay under data/.
Folder Path
Operational Role
data/live/
Landing zone for new VantagePoint CSVs.
data/processed/
Archive for processed CSVs (Auditable history).
python/ingest/
Core logic for DB entry and schema validation.
python/matching/
The IntelliScan similarity math engine.
python/reporting/
Decision support and statistical aggregation.
python/utilities/
PowerShell wrappers and database health scripts.
models/
SQLite databases (The "Brain" of the project).
...
AI MUST
Rule 3.2.1 (P_300 Thread Start): Read this architecture document at the start of every new P_300 thread. https://docs.google.com/document/d/1NNwDsbKjL5nV9q6qUmwVdkmM28y4KNLBu16LL3bHL50/edit?usp=sharing
Rule 3.2.2 (Status Summarization): Summarize current status before starting new work.
Rule 3.2.3 (Milestone State): Use documented milestone state, not assumptions.
Rule 3.2.4 (Fact/Proposal Distinction): Distinguish confirmed project facts from proposed next steps.
Rule 3.2.5 (Output Alignment): Keep outputs aligned to project folder and naming conventions.
Rule 3.2.6 (Catalog Bracketing): Execute Catalog Check-Out before any DB-touching operation and Check-In after.
Rule 3.2.7 (Canonical DB Write): Apply the Lock + Temp-DB + Atomic Move pattern for all write operations against the canonical DB.

AI MUST NOT
Rule 3.2.8 (Status Invention): Invent project status that is not documented.
Rule 3.2.9 (Architecture Loading Skip): Skip architecture loading on a new thread.
Rule 3.2.10 (Convention Change): Change schema or folder conventions without documenting the change.
Rule 3.2.11 (Thread Memory): Treat thread memory as authoritative when the architecture document says otherwise.
Rule 3.2.12 (Unauthorized Drift): Introduce a new parser, schema, workflow, or generalized implementation path when the approved task is to clone the validated POC exactly.
Rule 3.2.13 (Catalog Bracketing Skip): Skip the Check-Out / Check-In bracketing on any catalog operation.
Rule 3.2.14 (Master DB Write): Write directly to the master DB without going through `temp_working.db` first.
Rule 3.2.15 (The Full Rewrite Rule): If a change requires more than 2 lines of modification or spans multiple functions, Gemini MUST provide the entire script. No "insert here" instructions.
Rule 3.2.16 (The Versioned Header Rule): Every script must contain a standardized Header with Version, Date, and a Change Log.
Rule 3.2.17 (POC Drift Guardrail): No new parsers or workflows. We clone the proven SPY-first path exactly.
Rule 3.2.18 (Atomic DB Pattern): Any DB writes must use temp_working.db + Atomic Move. (Not applicable to the current read-only evaluation script, but locked for the next phase
se).


## **3.3 The P_300 Bootstrap Handshake (Session Initialization Rule)**

At the start of every new thread, the AI must execute the **P_300 Bootstrap Handshake** before accepting any task.   INIT starts the process.

**The Handshake Prompt:** Read P300_System_Architecture.md first. Next, explicitly review **Section 6 (Error Corrections Log)** to understand past failures and ensure you do not repeat errors that cause hallucination or drift. Perform the **Catalog Check-Out** (Section 2.7) to confirm the current state of `mmddyygeminicatalog.db`. Finally, summarize the current project status, identify the current milestone, restate the next approved objective, list any required reference files, and wait for confirmation or proceed only within those constraints. Treat the validated Multi-Symbol POC as the default implementation path and flag any new parser, schema, workflow, or generalized approach as POC drift unless explicitly approved.
...
* **NFR-5 Scalability:** SPY-first design must support later multi-symbol expansion.  This ha been implemented and superceded SPY-First Design.
...
## **5.1 Version History**
* **v1.19 — 2026-05-12**
*  Try to reconcile redundancies .  Opposing instructions .  By having Gemini analyze and rewrite Gemini Memory and Gem instructions for P-300.   Implemented Gemini  @docs to read P_300_System_Architecture MASTER and P_300_TASKS_LESSONS_MASTERt documents  
* Implemented Multi-Symbol Functionality to replace SPY-First POC. 
* Added EC057 … EC059
* **v1.18 — 2026-05-11**
 *   Add Pattern DailyWorkflow  need validation Found many flaws 
*   Added Issue  | ID-005 | Discovered I was spoofed into thinking add pattern was complete.  Working with Gemini creates a Typhoid Mary break  in the Architecture because of not contextualizing Skils, Gems and  Memory instructions with Architecture document. Does not learn from mistakes and truly needs MCP functionality .  This has truly taken more than a weeks work that was completely unnecessary.  If Gemini could be trained effectively.
* **v1.17 — 2026-05-10**
 *  Complete Daily Workflow into Flow Streams Add Pattern and Evaluate Trade (DailyWorkflow)
*    Section 2.6 Completely rewritten to reflect changes.
Workflow
Primary Logic
Purpose
Add Pattern
Assembly Line (PowerShell)
Safety-first "Write" operations. Ingests, processes, and audits database health.
Daily Evaluation
Integrated Engine (Python)
High-speed "Read" operations. Combines Similarity, Statistics, and Risk Configs into one report.

EC054-EC057 added - 

* **v1.16 — 2026-05-09**
  *  Rework Daily Workflow into Flow Streams Add Pattern and Evaluate Trade.  
  *. Still Testing Entirew Batch Flow for Both Flows
  * What should have take 1/2 hore was extednded to 4 hours major Gemini drifts .  Considering moving on from Gemini if this keeps up.
    * EC-049 ... EC-053
* **v1.15 — 2026-05-07**
  ...
  *. Deployed Production Code (Local Verification)
  **Module      Final Version    Local Path**
  * Config       v1.1.0          ...\parameters\optimization_config.json
  * IntelliScan  v2.2            ...\python\matching\intelliscan.py
  * Aggregator   v2.3            ...\python\reporting\aggregator.py
  * Sweep Tool   v1.3            ...\python\utilities\parameter_sweep.py
  * EC-043 ... EC-048
* **v1.14 — 2026-05-06**
...
|EC-049|  Directory Map Oversight: AI neglected to verify P_300_Directory_map.txt, issuing a backup command pointing to the project root instead of the designated models\ sub-directory. | Fixed | Rule Established: AI must explicitly verify the root and sub-directory paths in the provided Directory Map before proposing file system or PowerShell commands.
| EC-050 | Architectural Drift | | Rule: The ingest_manifest.json is the sole Source of Truth for keys. Never use "Ticker" or "Posture" if manifest defines symbol and psi. |
...
| EC-050 | Legacy File Conflict | | Rule (v5.2): Always purge \live\ data folder before conversion to prevent cross-contamination of different schemas. |
| EC-052 | Path Escaping | | Rule: Use Path(r""..."") or forward slashes for all Windows directory strings to prevent UnicodeEscape errors. |
| EC-054 | Extension Blindness | Fixed | Rule: Evaluation Engine must target .csv for converted live data and ignore .xlsx to prevent raw data corruption. |
| EC-055 | Workflow Disparity | Fixed | Rule: Add Pattern = Sequential (Safe Write); Daily Eval = Integrated (Fast Read). Keep PowerShell and Python roles distinct. |
| EC-056 | Hardcoded Indices | Fixed | Rule: Stop hunting SPY/QQQ files. Use P_010_RiskConfig.json as the Global Posture source of truth for sizing. |
...
|EC-058| Path Discovery,Fixed,Protocol: Hardcoded paths removed. All scripts must use db_utils.get_latest_catalog() to ensure 100% synchronization between Ingest and Math engines.
| EC-059| One Drive Bridge,Fixed,"Protocol: Converter v6.8 established as the ""Dual-Distributor"" to bridge D:\OneDrive source to both data/historical_patterns (Vault) and data/live (Daily Workflow)."
...
1. Execute the P_300 Bootstrap Handshake contained in INIT Process see P_300 Vanatge Point Pattern Recogniton from Perplexity Gem.
2. Read system architecture document and explicitly review the Error Log. document maintained during development session in Google docs https://docs.google.com/document/d/1NNwDsbKjL5nV9q6qUmwVdkmM28y4KNLBu16LL3bHL50/edit?usp=sharing
...
The canonical schema is defined by `models/mmddyygeminicatalog.db`.
...
| ID-005 |  Context loss across within a thread over 1/2  hour  | Highest | Need to restart new thread totally loses context.  Cannot follow an example interperts text as real value. Open (tracked in Error Log) |  Serious concern of moving foward with Gemini.
# ---
...
3. Perform **Catalog Check-Out** per Section 2.7 (verify `mmddyy
catalog.db` opens cleanly and record pre-op row counts).
...
## **11.3 Next Session Prompt**  See    Initialization Prompt Section    3.3 
