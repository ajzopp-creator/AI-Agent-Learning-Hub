...
| Preservation Reference CSV | `tests/preservation_371patterns_reference.csv` | Stage 5 regression test baseline |
...
| Source | `data/live/` (filename prefix `History Grid` or similar) |
...
- **`infrastructure/`** — All file reads/writes, SQLite queries, OneDrive paths, network calls. NO business logic. Returns raw data to the application layer. Examples: catalog reader/writer, VP CSV parser, P_010 risk config loader.
...
| OneDrive paths | `Path(os.environ["OneDrive"])` | Survives drive-letter changes and OneDrive folder migrations |
...
The eight-table normalized schema (defined in Section 9) is the primary protection against the bug classes that plagued the Gemini era. Three guarantees are enforced at the data layer:
...
- **NFR-6 Reproducibility:** Stage 5 re-ingestion of original source CSVs must reproduce the Stage 3 preservation reference labels.
...
- **8-table normalized schema** — adds `source_files`, `feature_sets`, `pattern_bars` (normalized window table). Drops `trade_outcomes` (merged into `forward_labels`).
...
| **EC-065** | **Symbols table duplicate entries (e.g., 'IVR' + 'History Grid (IVR)')** | **Resolved in v2.0** | Symbol parsing normalized to ticker-only; `UNIQUE` constraint enforced |
...
- Preservation export (371-pattern reference CSV)
...
- Empty `051126catalog.db` with 8-table schema
- `db_utils.py` glob update

## Stage 4 — Rebuild Pipeline A (Add Pattern)

- `config.py`, `schemas.py`, `domain/`, `infrastructure/`, `application/` for Pipeline A
- Rebuilt converter (no `tail(5)`, supports variable window)
- Rebuilt `ingest_vp_catalog.py` with proper FK handling
- Rewritten `ingest_manifest.json` for `pattern_bars` columns
- Normalization layer at ingest (pre-computed columns)
- Feature engineering layer (derived features in `pattern_features`)
- Forward labeler (5/7/10/15/20 horizons)
- Catalog summary / health check
- `verify_ingestion.py` — wraps Verify Temp + Atomic Move + Log steps

## Stage 5 — Re-Ingest Historical Patterns

- Pipeline A run against all `data/historical_patterns/*.csv` and `data/historical/Pattern_*.csv`
- Regression test against preservation reference CSV
...
- Confidence / Win-Rate (drives position size — e.g., 80% win rate → 2% risk; 60% win rate → 0.5% risk)
...
- OneDrive paths use `Path(os.environ["OneDrive"])`
...
- **Feature Set** — A named, versioned feature engineering definition (e.g., `baseline_5bar_v1`, `window_20bar_v1`)
...
- **Regression test:** Stage 5 re-ingestion of original source CSVs must reproduce the labels in `tests/preservation_371patterns_reference.csv` (within tolerance for label-method changes).
...
If for any reason the SKILL fails to load (e.g., session in a different tool that doesn't honor `.claude/skills/`), the operator types `INIT` or `P_300 INIT` to manually trigger the SIP.
...
| OneDrive Path Resolution | `Path(os.environ["OneDrive"])` |
...
├── tests/                             (regression tests + preservation reference)
└── tasks/
    ├── todo.md
    └── lessons.md
```
...
**End of P_300 System Architecture v2.0**
