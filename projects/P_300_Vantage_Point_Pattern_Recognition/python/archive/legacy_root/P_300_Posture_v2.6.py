...
#!/usr/bin/env python3
"""
Bullish Trend Pattern Project V2.5
...
Version History:
  ...
  V2.5 - Added CSV export and analysis summary for historical mode
"""
...
# ---------- Historical Results Export ----------

def save_historical_results(results: List[Dict], output_path: str = None) -> pd.DataFrame:
    """Save historical results to CSV for analysis."""
    df = pd.DataFrame(results)
    
    # Default path relative to script location
    if output_path is None:
        script_dir = Path(__file__).parent
        output_path = script_dir.parent / "data" / "processed" / "backtest_results.csv"
...
def main(mode: str = "live"):
    ...
    if mode == "live":
        ...
        df = save_historical_results(results)
        print(f"\nTop SPY DL scores:")
        print(df.nlargest(5, 'spy_score')[['date', 'spy_score', 'risk_mode']])
        print(f"\nRisk mode breakdown:")
        print(df['risk_mode'].value_counts())
