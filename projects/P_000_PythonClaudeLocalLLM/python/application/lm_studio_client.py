...
Auto-starts LM Studio if not already running.
"""

import asyncio
import logging
from typing import Optional, Dict, Any
from infrastructure.lm_studio_api import (
    load_model,
    unload_model,
    send_chat_request,
...
from infrastructure.lm_studio_launcher import ensure_lm_studio_running
...
class LMStudioClient:
    ...
    Handles:
    - Auto-starting LM Studio if not running
    ...
    async def startup(self) -> bool:
        """
        Initialize client — ensure LM Studio is running.
        Auto-starts LM Studio if needed, up to 30 seconds timeout.
        ...
        # Ensure LM Studio is running (auto-start if needed)
        is_running = await ensure_lm_studio_running()
        
        if not is_running:
            logger.error("Failed to start/connect to LM Studio")
            return False
        ...
        if self.is_healthy:
            ...
            logger.error("LM Studio health check failed after startup")
