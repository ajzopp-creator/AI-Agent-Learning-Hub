...
**Governed By:** P_000 (`P_000_SYSTEM_DOCUMENTATION.md` — master architecture reference) / `Agentic-Hub-Governance\work_orders\` (work order ledger, per `WO_COMPLETION_GATE.md`)
...
> **Purpose:** [One line — what this file contains]
> **Last Updated:** [Date]
```
...
| Python Environment | p140 conda — `C:\Users\Trader\.conda\envs\p140\python.exe` (shared Hub-wide, never a new venv) |
| Work Order Ledger | `Agentic-Hub-Governance\work_orders\WO-[Project ID]-*.md` — every WO needs a Completion Gate block before OWNER_DONE and an Independent Review before CLOSED |
...
| `CLAUDE.md` (optional — audit first) | Project root | Small, edited-in-place current-state file: architecture snapshot, key paths, locked decisions, active stage. Build one only once `tasks\todo.md` shows real accumulated depth — see P_000 doc Section 3.6. Not default-required for a new project. |
| `tasks\lessons.md` + `tasks\todo.md` (optional — audit first) | `[Project ID]\tasks\` | Append-only session/lesson logs. Live caps: `todo.md` ~500 lines/~100KB, `lessons.md` ~40 entries/~70KB — split to `_archive` siblings when a live file crosses its cap, nothing deleted. |
...
| Claude | Anthropic's AI assistant — core AI engine for this system |
...
[Data Input / Signal Source]
         |
         v
[AI Analysis Layer — Claude / LLM]
         |
         v
[Decision Engine — Rules / Scoring / Filters]
         |
         v
...
#### Component 1: [Name — e.g., Signal Source / Data Input]
...
#### Component 2: [Name — e.g., Analysis / Decision Engine]
- **Responsibility:** [What it does]
- **Inputs:** [What feeds into it]
- **Outputs:** [What it produces]
- **Tools Used:** [Claude, TOS, Python, etc.]
...
#### Component 3: [Name — e.g., Execution / Logging]
- **Responsibility:** [What it does]
- **Inputs:** [What feeds into it]
- **Outputs:** [What it produces]
- **Tools Used:** [Claude, TOS, Python, etc.]
...
```
[Project Root Name]
├── [Module 1 — e.g., Data Management]
│   ├── [Sub-component 1]
│   ├── [Sub-component 2]
│   └── [Sub-component 3]
├── [Module 2 — e.g., Analysis Engine]
│   ├── [Sub-component 1]
│   ├── [Sub-component 2]
│   └── [Sub-component 3]
├── [Module 3 — e.g., Decision Logic]
│   ├── [Sub-component 1]
│   └── [Sub-component 2]
└── [Module 4 — e.g., Output / Reporting]
    ├── [Sub-component 1]
    └── [Sub-component 2]
```
...
**Hub-standard folder layout (default for every new project):**

```
[Project ID]_[ProjectName]/
├── python/          # All Python scripts
├── tos_scripts/     # ThinkScript files (if applicable)
├── data/
...
**Optional, audit-first additions — not part of the default layout:**

```
[Project ID]_[ProjectName]/
├── CLAUDE.md         # current-state file — build only once real depth exists
└── tasks/
    ├── lessons.md    # append-only, numbered M-series entries
    └── todo.md       # append-only, one dated entry per session
```

See P_000 doc Section 3.6 (Working-State Doc Pattern) before adding these two — several existing Hub projects were audited and correctly left without them. Don't build `CLAUDE.md` as a starting scaffold; it earns its place once `todo.md` has grown enough to be worth summarizing.

**Shared, Hub-root resources this project may draw on (never duplicate locally):**
...
| LM Studio wrapper | `integrations\lm_studio\` |

Full canonical path table: `P_000_SYSTEM_DOCUMENTATION.md` Section 3, "Canonical Path Standards."
...
|---|---|---|---|
| Claude.ai | Primary AI engine — analysis, decisions, output | [Pro / Team / API] | Core reasoning layer |
| [ThinkorSwim] | [Market data, charting, execution] | [Latest] | [ThinkScript indicators] |
| Python | [Automation, data processing] | p140 conda env (`C:\Users\Trader\.conda\envs\p140\python.exe`) | Shared Hub-wide — never a project-local venv |
| [VantagePoint] | [ML signal source] | [Latest] | [If applicable] |
| [Grok / ChatGPT] | [Secondary AI reference] | [—] | [If applicable] |
| [VS Code] | [Code editor] | [Latest] | [If applicable] |
| [Excel / Google Sheets] | [Tracking / logging] | [—] | [If applicable] |
...
| Session Init Required | [Yes / No — paste prompt at start?] |
...
#### Prompt: [Name — e.g., Step 1 Signal Analysis]
```
...
#### Prompt: [Name — e.g., Step 2 Position Sizing]
```
[Paste full prompt template here]
```
**Purpose:** [What task this prompt performs]
...
**Claude MUST:**
- [Rule 1 — e.g., Always search project knowledge before answering]
- [Rule 2 — e.g., Never fabricate diagnostic values]
- [Rule 3]

**Claude MUST NOT:**
- [Rule 1 — e.g., Invent account balance values]
- [Rule 2 — e.g., Apply hierarchy assumptions across systems]
- [Rule 3]
...
#### NFR-4: [Other — e.g., Speed / Reliability / Security]
...
**Fixed:**
- [Correction to previous logic — reference Error Corrections Log if applicable]
...
*Document critical errors that were discovered and corrected. This section is permanent — errors are never deleted, only marked Resolved.*

**Purpose:** Prevent the same mistake from recurring. Any error corrected 2+ times MUST be documented here.

---

### Error: [Short Title — e.g., Account Balance Confusion]
...
- [Action 2 — e.g., Added rule to project knowledge]
- [Action 3 — e.g., Updated session prompt]
...
|---|---|---|
| [Title] | [Date] | [Outcome — e.g., Win rate improved 8%] |
| [Title] | [Date] | [Outcome] |
...
### 8.1 Primary Workflow: [Name — e.g., Signal Generation / Morning Setup]
...
**Steps:**
1. [Step 1 — e.g., Export TOS data / Run scan]
2. [Step 2 — e.g., Paste into Claude with STEP 1 prompt]
3. [Step 3 — e.g., Review output, verify verdict]
4. [Step 4 — e.g., Proceed to position sizing if BUY]
5. [Step 5 — e.g., Log result in tracker]

**Expected Output:**
[Describe what comes out of this workflow — format, decisions, next action]
...
### 8.2 Secondary Workflow: [Name — e.g., Position Sizing / Code Review]
...
### 8.3 Review Workflow: [Name — e.g., Evening Review / Weekly Strategy Check]
...
**Schema Rules:**
- [Critical rule 1 — e.g., Column order is locked, never reorder]
- [Critical rule 2 — e.g., Never insert placeholder dashes in computed columns]
- [Critical rule 3]

### 9.4 Data Integrity Rules

- [Rule 1 — e.g., Never fabricate values — if unknown, use "--"]
- [Rule 2 — e.g., Capture diagnostic values immediately when pasted]
- [Rule 3 — e.g., All outputs must be tab-delimited for Excel compatibility]
...
- [ ] [Check 1 — e.g., Account balance correct: $35,000]
- [ ] [Check 2 — e.g., Column order correct: PatternType before BreakoutVerdict]
- [ ] [Check 3 — e.g., Output is tab-delimited]
- [ ] [Check 4 — e.g., No fabricated diagnostic values]
- [ ] [Check 5 — e.g., Python output matches expected sample]
...
1. [Action 1 — e.g., Check market posture / Pull scan results]
2. [Action 2 — e.g., Export data from TOS / VantagePoint]
3. [Action 3 — e.g., Open Claude project, paste init prompt]

**[Main Session]** (~[X] min)
1. [Action 1 — e.g., Run Step 1 for each candidate]
2. [Action 2 — e.g., Run Step 2 for BUY signals]
3. [Action 3 — e.g., Paste option chain data for Step 3]
4. [Action 4 — e.g., Log results in tracker]

**[Post-Session / Evening Review]** (~[X] min)
1. [Action 1 — e.g., Update trade outcomes]
2. [Action 2 — e.g., Paste review prompt to Claude]
3. [Action 3 — e.g., Note any system observations for Enhancement Log]
...
### 11.5 Work Order Governance (Hub Standard)

*Every Hub project uses this from day one — not bolted on after the fact.*

- **Ledger:** `Agentic-Hub-Governance\work_orders\WO-[Project ID]-[Phase].[Seq].md` — single source of truth for what's open, in progress, or awaiting review.
- **Status flow:** PENDING → IN_PROGRESS → OWNER_DONE → CLOSED. OWNER_DONE means implemented, not done — it's still open until Independent Review.
- **Completion Gate:** before a WO can be set to OWNER_DONE, the Completion Gate checklist block (see `WO_COMPLETION_GATE.md`) must be copied into the WO in the *same edit* — never backfilled after the fact.
- **Independent Review Requirement:** the session that implements a WO cannot close it. A separate session re-reads the WO's Acceptance Criteria against the actual code/output and confirms each box before CLOSED.
- **Session-start check:** every session for this project starts by listing all open WOs (not just the newest) sorted by Phase+Seq — see `work-order-governance` skill. BLOCKED stops the session; PENDING warns and asks before proceeding.

Full governance rules: `Agentic-Hub-Governance\work_orders\WO_COMPLETION_GATE.md` and `P_000_SYSTEM_DOCUMENTATION.md` Section 3.4.
...
#### Issue: [System-specific issue — e.g., No signals generating / Scan returning wrong tickers]
...
#### Issue: [System-specific issue — e.g., Python script error / API timeout]
- **Symptoms:** [What you see]
- **Root Cause:** [Why it happens]
- **Solution:**
  1. [Step 1]
...
- Error log: [How you track errors — e.g., Error Corrections Log in this doc]
- Code outputs: [Local path / GitHub]

**How to audit a past decision:**
1. [Step — e.g., Filter tracker by date and SignalSource]
2. [Step — e.g., Check Comments column for diagnostic values]
3. [Step — e.g., Locate original Claude session if needed]
...
| [Term] | [Definition] |
| [Term] | [Definition] |
| Session Init | Session Initialization Prompt — pasted at the start of each Claude session to load context |
...
| Error Corrections Log | Section 6 of this document — permanent record of all discovered and fixed errors |
...
| **python-project-architecture SKILL.md** | `C:\Users\Trader\AI-Agent-Learning-Hub\.claude\skills\python-project-architecture\SKILL.md` | Hub-wide Python coding standards, layer rules, file delivery rules, documentation standards (cross-reference rule + artifact publish rule) -- read BEFORE writing any Python |
| **P_000_SYSTEM_DOCUMENTATION.md** | `C:\Users\Trader\AI-Agent-Learning-Hub\projects\P_000_PythonClaudeLocalLLM\docs\` | Master architecture reference — check here first for any Hub-wide design or environment question |
| **WO_COMPLETION_GATE.md** | `C:\Users\Trader\AI-Agent-Learning-Hub\Agentic-Hub-Governance\work_orders\` | Completion Gate checklist, Independent Review Requirement, canonical path standards — governs every WO closure Hub-wide |
...
```
# [System Name] — Key Configuration Parameters
# Last Updated: [Date]
...
**Template Last Updated:** 2026-08-20
**Template Applies To:** All Anthony Zoppi Claude Projects and Local LLM Python Projects (AI-Agent-Learning-Hub)
**Supersedes:** UNIVERSAL_PROJECT_TEMPLATE_v1_1 (2026-02-25) — archived, not deleted, at `docs\_archive\UNIVERSAL_PROJECT_TEMPLATE_v1_1.md`. Updated to reflect real Hub governance: WO ledger, Completion Gate, Independent Review, CLAUDE.md/tasks pattern, p140 environment default. Ref WO-P000-E8.001.
