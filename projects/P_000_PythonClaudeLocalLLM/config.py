...
# Each model has a display name and the exact model identifier as it appears in LM Studio
class ModelConfig(TypedDict):
    """Type definition for model configuration."""
    id: str  # Exact model identifier in LM Studio
    name: str  # Human-readable name
    vram_gb: int  # Typical VRAM usage
    use_case: str  # Primary purpose
...
TEMPERATURE_SETTINGS = {
    "analysis": 0.7,  # Market analysis, reasoning tasks
    "coding": 0.3,  # Code generation (more deterministic)
    "creative": 0.9,  # Creative or exploratory tasks
}

# Context length presets per model (tokens)
CONTEXT_LENGTH = {
    "primary": 16384,  # DeepSeek R1 14B
    "batch": 8192,  # Qwen 32B
    "long_context": 65536,  # Llama 4 Scout
}

# ── TASK-TO-MODEL ROUTING TABLE ───────────────────────────────────────────────
# Maps task types to the model tier that should handle them
TASK_ROUTING = {
    # Fast, real-time tasks → primary
