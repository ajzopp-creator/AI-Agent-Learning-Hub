...
**Owner:** Tony
**Status:** **ACTIVE** — Phases 1, 2, 3, 3.5, 4, 5.3 complete and live. Full daily pipeline (3→3.5→4→5.3) scheduled via Task Scheduler at 9:15 AM. Phase 5 KB mode live for .eml ingestion. Outlook OAuth2 is LIVE (Entries 013/014/015) — first login succeeded, `--check-imap-auth --account outlook` passes silently on refresh.
...
3. **Outlook OAuth2 first login.** Tony runs `cli.py --outlook-oauth-login` once (browser consent), then `cli.py --check-imap-auth --account outlook` to confirm. Code is built and tested (Entry 013); only the live human step remains.
...
6. **Yahoo retention policy.** Set the 10-day retention on Yahoo's `ExtractedNewsletterFolder` once it has real content (icloud/gmail already set).
...
*End of P_805 SYSTEM_DOCUMENTATION v2.6 — ACTIVE*
