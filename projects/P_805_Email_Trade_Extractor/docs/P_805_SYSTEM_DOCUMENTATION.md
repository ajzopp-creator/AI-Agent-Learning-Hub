...
- Real IMAP move of successfully-extracted messages to `ExtractedNewsletterFolder` (peer of Inbox) for iCloud, Gmail, and Yahoo (Phase 5.3, live). Outlook excluded — see Entry 011.
...
- Move messages for any account in `config.MOVE_SKIP_ACCOUNTS` (currently `outlook` — Entry 011).
...
└── python\
    ...
    ├── requirements.txt                      # pydantic, email-validator, python-dateutil, google-genai, python-dotenv, keyring
    ...
    │   └── imap_mover.py                     # IMAP connect/move/check_auth via keyring (Phase 5.3)
    ...
    │   ├── __init__.py
    │   ├── test_imap_mover.py                # Entry 010: dry-run must never call conn.create()
...
**Phase 5.3 — IMAP Move to ExtractedNewsletterFolder: ✅ COMPLETE and LIVE (2026-07-14)**
Real server-side move (not local mbox editing) of every message that produced at least one `TickerSignal`, via `COPY` + `STORE \Deleted` + `EXPUNGE` over `imaplib`. Credentials via `keyring` only (service `p805_imap`), never in config or logs. Destination folder auto-created per account if missing. Idempotent via `data\moved_messages.csv` audit log — already-`moved` messages are never retried; `dry_run` log entries don't block a later real attempt. Safety default `config.MOVE_DRY_RUN` rehearsed the full flow before going live (Entry 010 caught and fixed a dry-run-still-mutates-server bug in folder creation). `outlook` is permanently excluded via `config.MOVE_SKIP_ACCOUNTS` — that Microsoft 365 tenant rejects Basic Auth/plain IMAP `LOGIN` (Entry 011); building OAuth2 support is deferred, not on the current path. Standalone credential check: `cli.py --check-imap-auth [--account X]` (connect+login+logout only, safe to run any time). First live run: **18 moved** (8 icloud + 10 gmail), **1 not-found** (yahoo — message left INBOX between Phase 3 scan and move attempt, benign), **0 failed**. Tony set a 10-day retention policy on the destination folder in Thunderbird for icloud/gmail (yahoo pending first real content).
...
| `MOVE_SKIP_ACCOUNTS` | `{"outlook"}` | Accounts excluded from Phase 5.3 move entirely (Entry 011) |
...
### 12.1 Status at Close (2026-07-18, v2.2)
...
**Credential note:** IMAP passwords for icloud/gmail/outlook/yahoo live only in Windows Credential Manager via `keyring` (service `p805_imap`), set with `keyring.set_password('p805_imap', '<account>', '<app password>')` run by Tony directly at his terminal — never through Claude, never in any file.
...
3. **Sector weighting (data).** 34 of 59 senders still untagged — fill in as they accumulate ticker-producing history, or when Tony knows the newsletter's focus firsthand.
4. **parent_domain dedup.** New backlog item from this session — KryptonStreet/MarketCrux look like one publisher under two names. Not yet built.
5. **Yahoo retention policy.** Set the 10-day retention on Yahoo's `ExtractedNewsletterFolder` once it has real content (icloud/gmail already set).
6. **Outlook OAuth2 (deferred, not urgent).** If Outlook auto-filing becomes worth the scope, revisit `msal` + `AUTHENTICATE XOAUTH2` per Entry 011. Not currently planned.
...
*End of P_805 SYSTEM_DOCUMENTATION v2.2 — ACTIVE*
