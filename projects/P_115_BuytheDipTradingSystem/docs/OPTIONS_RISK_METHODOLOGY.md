...
## Live Options Data Lookup Sequence

Before pricing any options trade, pull live chain data in this order. Stop at the first source that returns data.

1. **TOS Platform** (PRIMARY) — Options chain is live in ThinkOrSwim. Pull directly from the platform before any web source.
2. **ChartExchange** — https://chartexchange.com/symbol/[TICKER]/optionchain/ — First web fallback. Returns bid/ask, OI, delta, IV by expiry.
3. **Yahoo Finance** — https://finance.yahoo.com/quote/[TICKER]/options/ — Second web fallback. Frequently returns 403 in automated calls; use browser if needed.
4. **Barchart / Nasdaq** — Third fallback. JavaScript-rendered; use browser only, not automated fetch.

**Rule:** If TOS is open, always try TOS first. Web lookups are fallbacks — not the default path.

---

## Options Chain Data Source Priority

**Source Order (use first that returns data):**

1. **TOS (ThinkorSwim)** -- ALWAYS PRIMARY
2. **ChartExchange** -- first web fallback
3. **Yahoo Finance** -- second web fallback
4. **Barchart / Nasdaq** -- last fallback

**Rule:** Stop at first source that returns usable options chain data. Do not aggregate across sources. TOS is always primary; web sources are fallbacks only when TOS data is unavailable.

**Usable data requires:** Bid/Ask both quoted, OI populated, Delta available (or computable from IV+strike+DTE).
