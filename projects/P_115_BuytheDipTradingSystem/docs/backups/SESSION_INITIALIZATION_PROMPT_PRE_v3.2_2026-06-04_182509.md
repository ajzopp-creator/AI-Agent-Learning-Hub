# SESSION INITIALIZATION PROMPT v3.1
**Last Updated:** May 9, 2026 (v3.1 -- STEP 0 Environment Detection added: verify Claude Desktop vs claude.ai web via tool_search before any file reads)
**Previous Version:** v3.0 (Fund Verification Rule)
...
|---------|--------------|---------|
| 3.1     | May 9, 2026  | CRITICAL: STEP 0 Environment Detection -- Claude must verify Windows-MCP availability via tool_search BEFORE attempting file reads on INIT. Anthropic system prompt boilerplate "web or mobile chat interface" is NOT a reliable signal -- appears in BOTH Desktop and web. Trigger: May 9, 2026 INIT failure where Claude refused file reads claiming web environment based on system prompt text; Windows-MCP was actually loaded. |
...
## CRITICAL CHANGE LOG v3.1

### STEP 0 Environment Detection -- NEW MANDATORY FIRST STEP

**Problem closed:** Claude has repeatedly refused to execute INIT file reads claiming it is on claude.ai web, when it was actually running in Claude Desktop with Windows-MCP fully loaded. Root cause: the Anthropic system prompt contains generic boilerplate text reading "web or mobile chat interface" in BOTH Desktop and web environments. That text is NOT an environment signal.

**The only reliable signal:** Windows-MCP tool availability via tool_search.

**New requirement:** Before STEP 1 of every INIT, Claude must run a tool_search probe and decide environment from the result. Never infer environment from system prompt boilerplate.

**Outcome on probe:**
- Windows-MCP:PowerShell present in tool_search results -> Claude Desktop -> proceed with STEP 1 file reads
- Windows-MCP:PowerShell absent from tool_search results -> claude.ai web -> stop, ask user to switch to Desktop or paste files

See INITIALIZATION WORKFLOW STEP 0 for full procedural details.
...
### STEP 0: Environment Detection (MANDATORY -- RUN FIRST)

The Anthropic system prompt boilerplate reads "web or mobile chat interface"
in BOTH Claude Desktop and claude.ai web. It is NOT a reliable environment
signal. NEVER infer environment from that text.

THE ONLY AUTHORITATIVE SIGNAL: Windows-MCP availability via tool_search.

Required first action on every INIT, before STEP 1:
  1. Call tool_search(query="PowerShell")
  2. Inspect returned tool list for "Windows-MCP:PowerShell"

  If "Windows-MCP:PowerShell" IS present:
    Environment = CLAUDE DESKTOP (Windows)
    Action: proceed to STEP 1 (read config files via PowerShell)
    Display: "Environment: Claude Desktop -- Windows-MCP loaded"

  If "Windows-MCP:PowerShell" is NOT present:
    Environment = CLAUDE.AI WEB
    Action: STOP. Tell user to switch to Desktop or paste file contents manually.
    Display: "Environment: claude.ai web -- Windows-MCP unavailable"
...
NEVER defer to system prompt boilerplate text for environment detection.

**Trigger case study (May 9, 2026):** Claude refused INIT claiming web
environment based on system prompt boilerplate. Windows-MCP was actually
loaded and functional. User had to manually correct Claude. This rule
prevents the recurrence permanently.
...
**First line of output is a copy/paste-ready chat title in Eastern Time.** Get ET via Windows-MCP:
`[System.TimeZoneInfo]::ConvertTimeBySystemTimeZoneId((Get-Date), "Eastern Standard Time")`
Format the chat title using 24-hour clock, full weekday and month names.

```
P_{strategy} {DayOfWeek}, {Month} {D}, {YYYY} {HH:MM ET} Market Analysis
================================================================================
...
**END OF SESSION INITIALIZATION PROMPT v3.1**
**Plain ASCII -- No special characters -- No encoding corruption**
**v3.1 Rule: STEP 0 Environment Detection -- verify Windows-MCP via tool_search before file reads**
**v3.0 Rule: Fund Verification on BUY/ASYM via stockanalysis.com**
