...
> **Linked Document:** P_115_P400_SIGNAL_PACKET_SCHEMA_v1_0.md
> **Location:** Project Knowledge / uploads
> **Purpose:** Signal packet schema for P_115 → P_400 handoff (Enhancements 1 & 2)
> **Last Updated:** 2026-06-02

> **Linked Document:** P_115_P400_Process_Evaluation_Recommendations.md
> **Location:** Project Knowledge / uploads
> **Purpose:** Architecture evaluation driving P_400 split (Enhancements 1 & 2)
> **Last Updated:** 2026-06-02

### Active Enhancements

#### Enhancement 1: P_400 Signal Packet Emitter (P_115 side)
- **Status:** Ready for implementation
- **Priority:** High
- **Target Date:** Q3 2026, Phase 1
- **Effort:** ~6–8 hours
- **Description:** After a BUY or ASYM verdict in STEP 1, P_115 writes a structured JSON signal file per `P_115_P400_SIGNAL_PACKET_SCHEMA_v1_0.md`. File lands in `TradeOrderManagement/signals/YYYY-MM-DD_SYMBOL_signal.json` via vault_interface. P_115 STEP 2 (sizing) remains inside P_115 for now. This enhancement produces the interface contract — no order logic moves yet.
- **Deliverable:** `signal_emitter.py` in `P_115_BuytheDipTradingSystem/python/`
- **Dependencies:** P_115_P400_SIGNAL_PACKET_SCHEMA_v1_0.md (complete)
- **Success Criteria:** Signal file written for every BUY/ASYM; P_400 reader can parse without error; ≥10 clean files before Enhancement 2 cutover

#### Enhancement 2: P_400 Full Handoff — Eliminate P_115 Order Management
- **Status:** Planned — depends on Enhancement 1 stable
- **Priority:** High
- **Target Date:** Q3 2026, Phase 2
- **Effort:** ~8–12 hours
- **Description:** P_400 reads signal files as exclusive STEP 2 input. P_115 STEP 2 (three-gate sizing, options viability, TP/SL) is deleted from P_115 and rebuilt inside P_400's order management engine. P_115 becomes signal-generation only — STEP 1 output is the terminal P_115 artifact.
- **Deliverable:** `signal_reader.py` (P_400 side) + removal of STEP 2 logic from P_115 session prompt and assistant instructions
- **Dependencies:** Enhancement 1 — signal files must flow reliably for ≥10 trades before cutover
- **Success Criteria:** Zero sizing calculations performed inside P_115; P_400 owns all order management; clean audit trail from signal_id through execution

#### Enhancement 3: Insider Data Integration *(formerly Enhancement 1)*
- **Status:** Planned
- **Priority:** Q3 2026, Priority 1 — July 2026
- **Description:** Weekly Dataroma check for insider purchases >$100K. CEO/CFO/Board buys as convergence tiebreaker for similar setups. Cross-reference with active watchlist.
- **Dependencies:** Q1-Q2 validation baseline (20+ trades by June 30)

#### Enhancement 4: 60-Min Analysis *(formerly Enhancement 2)*
- **Status:** Planned
- **Priority:** Q3 2026, Priority 2
- **Description:** Mini cup-and-handle confirmation inside daily handle for institutional entry timing. Adds second-timeframe confirmation layer to P_115 and P_118 ASYM signals.
- **Dependencies:** Q1-Q2 validation baseline

#### Enhancement 5: Distribution Day Tracker *(formerly Enhancement 3)*
- **Status:** Planned
- **Priority:** Q3 2026, Priority 3 — August 2026
- **Description:** Formal use of DistributionDayCount and FollowThroughDay columns with position sizing multiplier: Rally = 1.0×, Correction = 0.5×. Distribution day threshold: 4-5 days in 3-5 weeks = top signal.
- **Dependencies:** Q1-Q2 validation baseline

#### Enhancement 6: Z-Score Regime Council Integration *(formerly Enhancement 4)*
- **Status:** Planned — Phase 1 pilot September 2026
- **Priority:** Q3 2026, Priority 4
- **Description:** Add Z-Score pre-filter before full P_115 diagnostic run. Reads sumZZ, Regime, CMF_Z, OBV_Z from TOS overlay. GREEN/YELLOW/RED light gates candidates before 5-minute P_115 analysis. Currently observation-only; readings tracked in Comments.
- **Expected Benefit:** ~15 min/day saved; higher signal quality
- **Dependencies:** Z_SCORE_INTEGRATION_SPEC.md (v1.0 complete); Q1-Q2 validation baseline

#### Enhancement 7: Automated Position Alerts *(formerly Enhancement 5)*
- **Status:** Q4 2026 — requires stable baseline + validated strategies
- **Description:** Email/SMS when position approaching TP/SL; daily P&L summary; new Eddie Z picks auto-notification; Chaikin rating changes for held positions.

#### Enhancement 8: Performance Dashboard *(formerly Enhancement 6)*
- **Status:** Q4 2026 — after 50+ trades
- **Description:** Win rate by strategy, signal type, pattern, and market regime. Average R:R realized vs. planned. Drawdown analysis. Auto-update from tracker.

#### Enhancement 9: Correlation / Concentration Analysis *(formerly Enhancement 7)*
- **Status:** Q4 2026
- **Description:** Detect sector concentration risk; flag correlated positions (>0.7 correlation); suggest diversification; portfolio-level risk calculation.
...
| tracker_writer.py (P_800 schema v2.0) | Jun 2, 2026 | Reads Excel tracker, maps 27 columns, writes vault notes via write_to_vault(); 558 rows written on first run |
