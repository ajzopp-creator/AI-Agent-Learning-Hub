...
REM Find latest CSV file matching prefix
echo Searching for latest %FILE_PREFIX%*.csv file...
set LATEST_FILE=
for /f "delims=" %%a in ('dir /b /od "%TOS_FOLDER%\%FILE_PREFIX%*.csv" 2^>nul') do set LATEST_FILE=%%a

if "%LATEST_FILE%"=="" (
    echo.
    echo ERROR: No %FILE_PREFIX%*.csv files found in:
    echo   %TOS_FOLDER%
    echo.
    echo Please export from ThinkorSwim first!
    echo.
    pause
    exit /b 1
)
...
REM Run parser
"C:\Users\Trader\.conda\envs\p140\python.exe" P_020_TOS_Parser_v2.py "%CSV_PATH%"
