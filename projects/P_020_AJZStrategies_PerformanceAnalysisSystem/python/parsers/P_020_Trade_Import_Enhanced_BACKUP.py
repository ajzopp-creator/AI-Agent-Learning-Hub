...
def load_tracker_dashboard():
    ...
    try:
        if not os.path.exists(TRACKER_DASHBOARD_PATH):
            print(f"  âš  Warning: Tracker Dashboard not found at:")
            print(f"     {TRACKER_DASHBOARD_PATH}")
            print(f"  â†’ System names will default to 'TOS_Import'")
            return None
        ...
        if not all([symbol_col, date_col, signal_col]):
            print(f"  âš  Warning: Could not find required columns in Tracker Dashboard")
        ...
        print(f"  âœ“ Loaded Tracker Dashboard: {len(tracker)} trades")
        return tracker
        
    except Exception as e:
        print(f"  âš  Warning: Error loading Tracker Dashboard: {e}")
        print(f"  â†’ System names will default to 'TOS_Import'")
        return None
...
def match_system_name(symbol, trade_date, tracker_df):
    ...
    try:
        ...
        print(f"  âš  Error matching {symbol} on {trade_date}: {e}")
        return "TOS_Import"
    ...
    try:
        for encoding in ['utf-8', 'utf-8-sig', 'latin-1', 'cp1252']:
            try:
                ...
                print(f"  âœ“ Read CSV: {os.path.basename(csv_path)} ({len(df)} rows)")
                return df
...
def import_trades(csv_path, excel_path, trade_type="options", tracker_df=None):
    ...
    if df is None or df.empty:
        print(f"  âœ— ERROR: No data in CSV")
        return False
    ...
    if tracker_df is not None and 'System' in df.columns:
        ...
        print(f"  âœ“ Matched {matched}/{len(df)} trades to specific systems")
        if matched < len(df):
            print(f"  â†’ {len(df) - matched} trades remain as 'TOS_Import' (no Tracker match)")
    ...
    try:
        ...
        if LOG_SHEET_NAME in wb.sheetnames:
            ...
            ws = wb.active
        print(f"  âœ“ Opened sheet: {ws.title}")
    except Exception as e:
        print(f"  âœ— ERROR: Could not open workbook: {e}")
        return False
    ...
    for idx, row in df.iterrows():
        ...
        if excel_row > 101:
            print(f"  âš  Warning: More than 100 rows. Truncating at row 101.")
            break
    ...
    try:
        ...
        print(f"  âœ— ERROR: Could not save workbook: {e}")
    ...
    print(f"\n{'='*60}")
    print(f"âœ… SUCCESS: {rows_written} trades imported to {os.path.basename(excel_path)}")
    print(f"{'='*60}")
...
def main():
    ...
    while True:
        ...
        print("\n   Select import type:\n")
        print("   [1] Options â†’ Paper Log")
        print("   [2] Options â†’ Live Log")
        print("   [3] Stocks  â†’ Paper Log")
        print("   [4] Stocks  â†’ Live Log")
        print("   [5] Import ALL â†’ Paper")
        print("   [6] Import ALL â†’ Live")
        ...
        if choice == "1":
            ...
            if csv:
                ...
                print(f"\n  âœ— No OPTIONS CSV found in {PAPER_INPUT_FOLDER}")
            ...
            if csv:
                ...
                print(f"\n  âœ— No OPTIONS CSV found in {LIVE_INPUT_FOLDER}")
            ...
            if csv:
                ...
                print(f"\n  âœ— No STOCKS CSV found in {PAPER_INPUT_FOLDER}")
            ...
            if csv:
                ...
                print(f"\n  âœ— No STOCKS CSV found in {LIVE_INPUT_FOLDER}")
