#!/usr/bin/env python3
"""
P_020_TOS_Parser_v2.py - Enhanced TOS Account Statement Parser
Parses ThinkorSwim CSV exports and maps to P_020/D_020 Stock/Options log format
...
def group_trades_into_positions(df):
    ...
    Same REF # = Same order (sum quantities and commissions).
    Matches BOT (entries) with multiple SOLD (exits).
    """
    ...
    # Step 1: Group by REF # to combine multiple fills of same order
    # Same REF # = Same order, need to SUM quantities and commissions
    aggregated_trades = []
    
    for ref_num in df['REF #'].unique():
        ref_trades = df[df['REF #'] == ref_num]
        
        if len(ref_trades) == 1:
            # Single fill, use as-is
            aggregated_trades.append(ref_trades.iloc[0].to_dict())
        else:
            ...
            aggregated_trades.append(first_trade)
    
    # Convert back to DataFrame
    df_aggregated = pd.DataFrame(aggregated_trades)
    
    positions = []
    
    # Step 2: Group by symbol + option details
    for symbol in df_aggregated['symbol'].unique():
        symbol_trades = df_aggregated[df_aggregated['symbol'] == symbol].copy()
...
def _match_entries_exits(trades_df):
    ...
    Handles multiple exits (Exit #1 and Exit #2).
    """
    ...
    for _, entry in entries.iterrows():
        position = {
            ...
            # Financial (round commission to 2 decimal places)
            'Comm.': round(abs(entry['total_commission']), 2),
        ...
        # Find ALL matching exits (not just first one)
        matching_exits = exits[
            (exits['symbol'] == entry['symbol']) &
            (pd.to_datetime(exits['DATE'], format='%m/%d/%y') >= pd.to_datetime(entry['DATE'], format='%m/%d/%y'))
        ].sort_values('datetime')
        
        if not matching_exits.empty:
            # Exit #1 (first exit)
            exit1 = matching_exits.iloc[0]
            
            position['Exit #1'] = exit1['price']
            position['# Exited'] = exit1['quantity']
            position['Exit Date'] = exit1['DATE']
            
            # Calculate days for Exit #1
            entry_date = pd.to_datetime(entry['DATE'], format='%m/%d/%y')
            exit1_date = pd.to_datetime(exit1['DATE'], format='%m/%d/%y')
            position['# of Days'] = (exit1_date - entry_date).days
            
            # Add exit #1 commission
            position['Comm.'] = round(position['Comm.'] + abs(exit1['total_commission']), 2)
            
            # Exit #2 (second exit, if exists)
            if len(matching_exits) > 1:
                exit2 = matching_exits.iloc[1]
                
                position['Exit #2'] = exit2['price']
                position['# Exited2'] = exit2['quantity']
                position['Exit Date3'] = exit2['DATE']
                
                # Calculate days for Exit #2
                exit2_date = pd.to_datetime(exit2['DATE'], format='%m/%d/%y')
                position['# of Days4'] = (exit2_date - entry_date).days
                
                # Add exit #2 commission
                position['Comm.'] = round(position['Comm.'] + abs(exit2['total_commission']), 2)
...
def format_for_stock_log(positions_df):
    ...
    output = pd.DataFrame({
        ...
        'Exit #2': stocks_df['Exit #2'],
        '# Exited2': stocks_df['# Exited2'],
        'Exit Date3': stocks_df['Exit Date3'],
        '# of Days4': stocks_df['# of Days4'],
        'Comm.': stocks_df['Comm.'],
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python P_020_TOS_Parser_v2.py <tos_account_statement.csv>")
        print("\nExample:")
        print("  python P_020_TOS_Parser_v2.py 2026-01-28_AJZ_Strategies_YTD_AccountStatement.csv")
        sys.exit(1)
    ...
    print(f"  âœ“ Option trades: {options_count}")
    print(f"  âœ“ Stock trades: {stocks_count}")
    if unknown_count > 0:
        print(f"  âš  Unparsed: {unknown_count}")
    
    # Step 3: Group into positions
    print("\nStep 3: Matching entries with exits...")
    positions_df = group_trades_into_positions(parsed_df)
    print(f"  âœ“ Created {len(positions_df)} position records")
    ...
    if not options_output.empty:
        ...
        print(f"  âœ“ Options: {len(options_output)} trades â†’ {options_file.name}")
    else:
    ...
    if not stocks_output.empty:
        ...
        print(f"  âœ“ Stocks: {len(stocks_output)} trades â†’ {stocks_file.name}")
    else:
    ...
    if not options_output.empty:
        print(f"  ðŸ“Š {options_file.name}")
        print(f"     â†’ Paste into: P_020_2026_AJZ_Strategies_Options_Log_v1.xlsx (LIVE)")
        print(f"                 or: D_020__Paper_Options_Log_v2.xlsx (PAPER)")
    if not stocks_output.empty:
        print(f"  ðŸ“ˆ {stocks_file.name}")
        print(f"     â†’ Paste into: P_020_2026_AJZ_Strategies_Stock_Log_v1.xlsx (LIVE)")
        print(f"                 or: D_020__Paper_Stock_Log-V2.xlsx (PAPER)")
    
    print("\nðŸ’¡ TIP: Open CSV files, copy all rows, paste into Excel starting at next empty row")
    print()
